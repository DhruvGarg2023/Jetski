...
import { GitBranch, Code, ExternalLink, CalendarDays, Code2 } from "lucide-react";
      ...
      {allRepos.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {allRepos.map((repo, index) => (
            ...
            >
              <Card className="flex flex-col h-full glass-subtle border-white/5 overflow-hidden transition-all duration-300 card-hover">
                ...
                <CardHeader className="relative z-10">
                  ...
                  <CardTitle className="text-xl font-bold truncate mt-2">
                    {repo.fullName}
                  </CardTitle>
                </CardHeader>
                <CardContent className="flex-1 relative z-10">
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-2.5 text-sm text-muted-foreground bg-black/20 p-2.5 rounded-lg border border-white/5">
                    ...
                    <div className="flex items-center gap-2.5 text-sm text-muted-foreground p-1">
                      <CalendarDays className="h-4 w-4 text-primary/70" /> 
                      <span>Connected {formatDistanceToNow(new Date(repo.createdAt), { addSuffix: true })}</span>
                    </div>
                  </div>
                </CardContent>
                <CardFooter className="pt-4 border-t border-white/5 relative z-10 mt-auto">
                  <Link href={`/repositories/${repo.id}`} className={buttonVariants({ variant: "secondary", className: "w-full shadow-sm shadow-black/50 group-hover:bg-primary group-hover:text-primary-foreground group-hover:shadow-primary/25 transition-all duration-300" })}>
                    View Details
